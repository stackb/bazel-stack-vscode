...
describe(BzlFeatureName, function () {
  ...
  describe('Configuration', () => {
    it('setServerAddresses', async () => {
      ...
      expect(grpcConfig.command).eql(['serve', '--address=localhost:9090']);
    });
  });
});

export function createLicensesServiceServer(
  address: string,
  status: grpc.status,
  license?: License
): Promise<grpc.Server> {
  return new Promise<grpc.Server>((resolve, reject) => {
    ...
    server.addService(licenseProtos.build.stack.license.v1beta1.Licenses.service, {
      ...
      renew: (
        ...
        callback: (err: grpc.ServiceError | null, resp?: RenewLicenseResponse) => void
      ) => {
