...
import { BazelDocConfiguration } from '../../bazeldoc/configuration';
...
suite(BazelDocFeatureName, function () {
  ...
  suiteSetup(async () => {
    ...
    const emitter = new vscode.EventEmitter<BazelDocConfiguration>();
    hoverProvider = new BazelDocGroupHover(emitter.event);
    emitter.fire({
