...
export class BazelDocGroupHover implements vscode.HoverProvider, vscode.Disposable {
  ...
  private groups: Map<string, BazelDocGroup> | undefined;
  private cfg: BazelDocConfiguration | undefined;

  constructor(onDidConfigurationChange: vscode.Event<BazelDocConfiguration>) {
    onDidConfigurationChange(this.handleConfiguration, this, this.disposables);
    this.disposables.push(
      vscode.languages.registerHoverProvider(
        [{ language: 'bazel' }, { language: 'starlark' }],
        this
      )
    );
  }

  private handleConfiguration(cfg: BazelDocConfiguration) {
    this.cfg = cfg;
 ...
 * @param group The group to which the item belongs
 */
export function makeBazelDocGroupHoverMarkdown(
  item: string,
  group: BazelDocGroup,
  baseUrl: string
