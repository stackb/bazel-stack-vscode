...
export class BzlIoReleaseAssetDownloader {
     ...
     */
    async getOrDownloadFile(ctx: vscode.ExtensionContext, mode: number, token: vscode.CancellationToken): Promise<vscode.Uri> {
        ...
        fileUri = await vscode.window.withProgress({
            ...
            return this.downloaderApi.downloadFile(vscode.Uri.parse(url), filename, ctx, token, (downloaded: number, totalBytes: number | undefined) => {
     ...
     * @returns 
     */
    static async fromConfiguration(cfg: BzlIoReleaseAssetConfiguration): Promise<BzlIoReleaseAssetDownloader> {
