...
export class BzlFeature implements vscode.Disposable {
  ...
  async setupBazelActivity(ctx: vscode.ExtensionContext, cfg: BzlConfiguration) {
    ...
    const commandRunner = this.add(
      new BzlServerCommandRunner(cfg.commandTask, this.onDidBzlClientChange.event)
    );
    ...
    const repositoryListView = this.add(new BzlRepositoryListView(this.onDidBzlClientChange.event));

    this.add(
      new BzlCommandHistoryView(
        this.onDidBzlClientChange.event,
    ...
    const workspaceListView = this.add(
      new BzlWorkspaceListView(
        this.onDidBzlClientChange.event,
        repositoryListView.onDidChangeCurrentRepository.event
      )
    );
    ...
    this.add(
      new BzlServerView(bzlProto, codesearchProto, cfg.server.remotes, this.onDidBzlClientChange)
    );
    ...
    this.add(
      this.onDidServerDoNotRestart.event(msg => {
  ...
  async tryConnectServer(cfg: BzlServerConfiguration, attempts: number): Promise<void> {
    if (attempts > 3) {
      return Promise.reject(
        `could not connect to bzl: too many failed attempts to ${cfg.address}.  Server will not be restarted.`
      );
    }
    try {
  ...
  async restartServer(cfg: BzlServerConfiguration, attempts: number): Promise<void> {
    ...
    const server = (this.server = this.add(
      new BzlServer(this.onDidServerDoNotRestart, cfg.executable, cfg.command)
    ));
  ...
  setupStackBuildActivity(ctx: vscode.ExtensionContext, cfg: BzlConfiguration) {
    ...
    this.disposables.push(
      new BzlLicenseRenewer(
        this.onDidBzlLicenseExpire,
        this.onDidBzlLicenseTokenChange,
        licenseClient
      )
    );
