...
export class BzlWorkspaceListView extends BzlClientTreeDataProvider<WorkspaceItem> {
  ...
  public onDidChangeCurrentExternalWorkspace: vscode.EventEmitter<ExternalWorkspace | undefined> =
    new vscode.EventEmitter<ExternalWorkspace | undefined>();
  ...
  protected async getRootItems(): Promise<WorkspaceItem[] | undefined> {
    ...
    const externals = (this.externals = await this.client?.listExternalWorkspaces(
  ...
  private createExternalWorkspaceMetadataItems(
    externals: ExternalWorkspace[]
    ...
    items.push(
      new DefaultWorkspaceItem(
        Container.media(
          this.currentExternalWorkspace ? MediaIconName.WorkspaceGray : MediaIconName.Workspace
        )
      )
    );
    ...
    for (const external of externals) {
      ...
      if (
        external.relativeLocation?.startsWith('/DEFAULT.WORKSPACE') &&
        external.name !== 'bazel_tools'
      ) {
        continue;
      }
      ...
      const icon = Container.media(
        this.currentExternalWorkspace?.id === external.id
          ? MediaIconName.Workspace
          : MediaIconName.WorkspaceGray
      );
...
export class WorkspaceItem extends vscode.TreeItem {
  constructor(public label: string, public iconPath: vscode.Uri) {
    super(label);
    this.command = {
