...
import {
  ...
  ContextValue,
  FileName,
  ThemeIconCircleOutline,
  ThemeIconDebugContinue,
 ...
 */
export class BzlCommandHistoryView
  extends BzlClientTreeDataProvider<CommandHistoryItem>
  implements CommandCodeLensProvider
{
  ...
  async handleCommandAdd(): Promise<void> {
    ...
    const pickCommand: InputStep = async input => {
      const picked = await input.showQuickPick({
        ...
        totalSteps: 2,
        step: 1,
        items: Array.from(bazelCommands.values()).map(name => {
          return { label: name };
        }),
...
export class CommandHistoryItem extends vscode.TreeItem {
  ...
  constructor(readonly history: CommandHistory) {
    ...
    this.updated = luxon.DateTime.fromSeconds(
      Long.fromValue(history.updateTime?.seconds as Long).toNumber()
    );
...
function registerBazelCommandCodeLensProviders(
  registry: ICommandCodeLensProviderRegistry,
