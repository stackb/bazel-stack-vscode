...
export class CreateSubscriptionFlow extends GRPCResponseFlow<Subscription> {
    ...
    getInternal(): Promise<{response: Subscription | undefined, error: grpc.ServiceError | undefined}> {
        return new Promise((resolve, _) => {
            this.subscriptionsClient.CreateSubscription(this.request, this.getGrpcMetadata(), (err?: grpc.ServiceError, resp?: Subscription) => {
                resolve({
                    response: resp,
                    error: err,
                });
