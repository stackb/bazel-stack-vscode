...
export abstract class GRPCResponseFlow<T> {
    ...
    abstract getInternal(): Promise<{response: T | undefined, error: grpc.ServiceError | undefined}>;

    async handleServiceError(status: grpc.ServiceError): Promise<void> {
        vscode.window.showErrorMessage(`${this.name} service error: ${status.message} (${status.code})`);
