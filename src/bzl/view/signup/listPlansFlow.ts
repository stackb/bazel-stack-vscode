...
export class ListPlansFlow extends GRPCResponseFlow<ListPlansResponse> {
    ...
    getInternal(): Promise<{response: ListPlansResponse | undefined, error: grpc.ServiceError | undefined}> {
        return new Promise((resolve, _) => {
            this.plansClient.List({}, this.getGrpcMetadata(), (err?: grpc.ServiceError, resp?: ListPlansResponse) => {
                resolve({
                    response: resp,
                    error: err,
                });
