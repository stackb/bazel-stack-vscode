...
export class GitHubOAuthFlow implements vscode.Disposable {
  ...
  public async getExternalCallbackUri(): Promise<vscode.Uri> {
    return vscode.env.asExternalUri(
      vscode.Uri.parse(`${vscode.env.uriScheme}://StackBuild.bazel-stack-vscode/did-authenticate`)
    );
  }

  public async login(
    state: string = uuid(),
    timeoutSeconds = 60 * 10 /* 10min */,
    ...
    const uri = vscode.Uri.parse(
      `${this.baseUrl}?redirect_uri=${encodeURIComponent(
    ...
    return Promise.race([
      utils.promiseFromEvent(
        this.uriHandler.event,
        GitHubOAuthFlow.extractJWTFromCallbackURI(state)
      ),
  ...
  static extractJWTFromCallbackURI: (state: string) => utils.PromiseAdapter<vscode.Uri, string> =
