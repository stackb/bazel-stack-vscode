...
export class BzlSignup implements vscode.Disposable {
  ...
  constructor(
    ...
    this.disposables.push(
      vscode.commands.registerCommand(CommandName.SignupStart, this.handleCommandSignupStart, this)
    );
  }
...
export class BzlGetStarted implements vscode.Disposable {
  ...
  async getStarted(): Promise<void> {
    ...
    await this.panel.render({
      ...
      heading: '',
      subheading:
        '<a href="https://bzl.io" style="color: var(--vscode-editor-foreground)">Boost Productivity</a>',
      image: {
        url: 'https://user-images.githubusercontent.com/50580/95278334-37481500-080d-11eb-8931-88905c1c3d51.gif',
      },
      lead:
        '<p>Sign-Up to get the most out your vscode+bazel experience.</p>' +
      ...
      features: [
        ...
        },
        {
          ...
          highlights: [{ text: 'Launch the sign-up webview' }, { text: 'Show account details' }],
        },
        {
  ...
  async tryListPlans(jwt: string = this.jwt): Promise<any> {
    ...
    const flow = new ListPlansFlow(this.plansClient, jwt, (plans: Plan[]) => {
      this.plans = plans;
  ...
  async trySelectPlan(
    jwt: string = this.jwt,
    plans: Plan[] | undefined = this.plans
  ): Promise<any> {
    if (!jwt) {
      return this.getStarted();
    }
      ...
      this.panel.render({
        ...
        cards: plans.map(p => {
          return {
            name: p.name!,
            image:
              'https://user-images.githubusercontent.com/50580/78734937-bfa13800-7906-11ea-8e94-f76af76a65ea.png',
  ...
  static extractPlanName: () => utils.PromiseAdapter<string, string> =
    () => async (name, resolve, reject) => {
      resolve(name);
    };

  async tryCollectPaymentDetails(
    jwt: string = this.jwt,
          ...
          inputs: [
            {
              ...
              onchange: async (value: string) => {
                ...
                return (
                  (result.isPotentiallyValid ? 'Incomplete ' : 'Invalid ') +
                  (result.card ? result.card.type : 'card') +
                  ' number'
                );
              },
            },
  ...
  async tryConfirm(
    jwt: string = this.jwt,
    plan: Plan | undefined = this.selectedPlan,
    card: Card | undefined = this.selectedCard
  ): Promise<any> {
    if (!jwt) {
      return this.getStarted();
    }
...
function formatPlanAmount(
  amount: Long | string | number | undefined,
