...
export class BzlAccountView extends GrpcTreeDataProvider<AccountItem> {
  ...
  constructor(
    ...
    this.disposables.push(
      onDidBzlLicenseTokenChange.event(token => {
...
function createLicenseItems(lic: License): AccountItem[] {
  ...
  return [
    new AccountItem(AccountItemName.ID, `${lic.id}`, 'Registered user ID', lic.avatarUrl),
    new AccountItem(AccountItemName.Name, `${lic.name}`, 'Registered user name'),
    new AccountItem(AccountItemName.Email, `${lic.email}`, 'Registered user email address'),
    new AccountItem(
      AccountItemName.Subscription,
      `${lic.subscriptionName}`,
      'Name of the subscription you are registered under'
    ),
    new AccountItem(AccountItemName.Exp, `${dt.toISODate()}`, 'Expiration date of this license'),
  ];
}
