...
export class CodeSearchCodeLens implements CommandCodeLensProvider, vscode.Disposable {
  ...
  registerCommands() {
    this.disposables.push(
      vscode.commands.registerCommand(CommandName.CodeSearchIndex, this.handleCodesearchIndex, this)
    );
    this.disposables.push(
      vscode.commands.registerCommand(
        CommandName.CodeSearchSearch,
        this.handleCodesearchSearch,
        this
      )
    );
  }
  ...
  checkCodelensPreconditions(command: string): boolean {
    ...
    if (!client) {
      vscode.window.showWarningMessage(
        `Cannot execute command "${command}" (bzl client not active)`
      );
      return false;
    }
    ...
    if (!ws) {
      vscode.window.showWarningMessage(
        `Cannot execute command "${command}" (no active bazel workspace)`
      );
      return false;
    }
    return true;
  ...
  async createScope(
    opts: CodesearchIndexOptions,
    client: BzlCodesearch,
    ws: Workspace,
    ...
    return vscode.window
      .withProgress<void>(
        ...
        async (
          ...
          return client
            .createScope(request, async (response: CreateScopeResponse) => {
