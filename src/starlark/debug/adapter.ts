...
// StarlarkDebugSession implements a DAP debug adapter to talk to the editor.
//
...
//
//      Editor                StarlarkDebugSession                 Delve
//  +------------+        +--------------+-----------+         +------------+
//  | DAP Client | <====> | DebugSession | DAPClient |  <====> | DAP Server |
//  +------------+        +--------------+-----------+         +------------+
export class StarlarkDebugSession extends LoggingDebugSession {
    ...
    private starlarkClient: StarlarkClient | undefined = undefined;

    // // Child process used to track debugee launched without debugging (noDebug
    // // mode). Either debugProcess or dlvClient are undefined.
    // private debugProcess: ChildProcess = undefined;
    ...
    protected launchRequest(
        ...
        const dlvClient = this.starlarkClient = new StarlarkClient(args);
        ...
        dlvClient.on('connected', () => {
            dlvClient.send(request);
        });

        dlvClient.on('close', (rc) => {
            if (rc !== 0) {
                // TODO: define error constants
                // https://github.com/golang/vscode-go/issues/305
                this.sendErrorResponse(
        ...
        dlvClient.on('event', (event) => {
            this.sendEvent(event);
        });

        dlvClient.on('response', (resp) => {
    ...
    protected attachRequest(
        ...
        this.starlarkClient!.send(request);
    }
    ...
    protected disconnectRequest(
        ...
        if (this.starlarkClient) {
            // Forward this DisconnectRequest to Delve.
            this.starlarkClient.send(request);
        } else {
            logError('both debug process and dlv client are undefined');
...
// StarlarkClient provides a DAP client to talk to a DAP server in Delve.
//
...
class StarlarkClient extends DAPClient {
    ...
    constructor(launchArgs: LaunchRequestArguments) {
        ...
        if (!fs.existsSync(dlvPath)) {
            log(
                `Couldn't find dlv at the  path, ${process.env['PATH']}`
            );
            throw new Error(
                'Cannot find dapstar debugger.  Install is by {TODO}'
            );
        }
        ...
        const child = this.debugProcess = spawn(dlvPath, dlvArgs, {
        ...
        child.on('close', (rc) => {
            if (rc) {
                logError(`Process exiting with code: ${rc} signal: ${child.killed}`);
            } else {
                log(`Process exiting normally ${child.killed}`);
            }
            this.emit('close', rc);
        });
